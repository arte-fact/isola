...
pub fn run() -> Result<(), IsolaError> {
    #[cfg(target_os = "linux")]
    {
        ...
        let status = std::process::Command::new("sudo")
            ...
            .map_err(|e| {
                IsolaError::ConfigError(format!("Failed to write AppArmor profile: {e}"))
            })?;
        ...
        let reload = std::process::Command::new("sudo")
            ...
            .status()
            .map_err(|e| {
                IsolaError::ConfigError(format!("Failed to reload AppArmor profile: {e}"))
            })?;
    ...
    {
        eprintln!("setup-host is only needed on Linux (for AppArmor user namespace support).");
        Ok(())
    }
}
