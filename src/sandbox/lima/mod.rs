...
impl LimaBackend {
        ...
        Command::new("limactl")
            ...
            .and_then(|o| {
                if o.status.success() {
                    ...
                    Some(text.lines().any(|line| line.contains(&format!("\"name\":\"{vm_name}\""))))
                } else {
                    None
                }
            })
    ...
    fn build_setup_files_script(name: &str, environments: &[String], isolation_desc: &str) -> String {
...
impl SandboxBackend for LimaBackend {
    ...
    fn write_sandbox_files(&self, name: &str, environments: &[String]) -> Result<(), IsolaError> {
        let script = Self::build_setup_files_script(name, environments, self.isolation_description());
    ...
    fn enter_interactive(
        ...
        let status = if shell {
            ...
            let check_cmd = format!(
                "PATH={sandbox_path} command -v claude >/dev/null 2>&1"
            );
            ...
            if has_claude {
                ...
                eprintln!("  Install with: isola shell {name}  then  npm install -g @anthropic-ai/claude-code");
                let cmd = format!(
    ...
    fn build_provision_script(&self, environments: &[String]) -> String {
        let mut script = rootfs::build_provision_script(environments);
